...
export class Migration20240407212135 extends Migration {

  async up(): Promise<void> {
    this.addSql('create table "product" ("uuid" uuid not null, "title" varchar(255) not null, "description" varchar(255) not null, "price" int not null, constraint "product_pkey" primary key ("uuid"));');

    this.addSql('create table "user" ("uuid" uuid not null, constraint "user_pkey" primary key ("uuid"));');

    this.addSql('create table "cart" ("uuid" uuid not null, "user_uuid" uuid not null, "total" int not null, constraint "cart_pkey" primary key ("uuid"));');
    this.addSql('alter table "cart" add constraint "cart_user_uuid_unique" unique ("user_uuid");');

    this.addSql('create table "order" ("id" uuid not null, "user_uuid" uuid not null, "cart_uuid" uuid not null, "payment_type" varchar(255) not null, "payment_address" varchar(255) null, "credit_card" varchar(255) null, "delivery_type" varchar(255) not null, "delivery_address" varchar(255) not null, "comments" varchar(255) not null, "status" varchar(255) not null, "total" int not null, constraint "order_pkey" primary key ("id"));');

    this.addSql('create table "cart_item" ("uuid" uuid not null, "cart_uuid" uuid not null, "product_uuid" uuid not null, "order_id" uuid null, "count" int not null, constraint "cart_item_pkey" primary key ("uuid"));');
